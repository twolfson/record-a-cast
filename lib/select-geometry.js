...
function selectGeometry(mainParams, cb) {
  ...
  var browserWindow = new BrowserWindow({
    frame: false,
    height: roundToDivisor(600, heightDivisor),
    width: roundToDivisor(800, widthDivisor),
    transparent: true
  });
  ...
  browserWindow.on('resize', function handleResize () {
    ...
    bounds.width = roundToDivisor(bounds.width, widthDivisor);
    bounds.height = roundToDivisor(bounds.height, heightDivisor);
  ...
  function handleResizeBy(evt, params) {
    ...
    var amt;
    if (params.top) { amt = roundToDivisor(params.top, heightDivisor); bounds.y -= amt; bounds.height += amt; }
    if (params.bottom) { bounds.height += roundToDivisor(params.bottom, heightDivisor); }
    if (params.left) { amt = roundToDivisor(params.left, widthDivisor); bounds.x -= amt; bounds.width += amt; }
    if (params.right) { bounds.width += roundToDivisor(params.right, widthDivisor); }
    browserWindow.setBounds(bounds);
    // jscs:enable
