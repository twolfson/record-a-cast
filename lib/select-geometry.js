...
// Define our function
function selectGeometry(params, cb) {
  // Verify we received width divisor and height divisor
  assert(params.widthDivisor && !isNaN(params.widthDivisor));
  assert(params.heightDivisor && !isNaN(params.heightDivisor));

  // Create a new transparent window
  // TODO: Occupy full viewport like Shutter?
  var browserWindow = new BrowserWindow({
    frame: false,
    height: 600 - (600 % params.heightDivisor),
    width: 800 - (800 % params.widthDivisor),
    transparent: true
  });
  ...
  function handleResizeBy(evt, params) {
    ...
    if (params.top) { bounds.y -= params.top; bounds.height += params.top; }
    if (params.bottom) { bounds.height += params.bottom; }
    if (params.left) { bounds.x -= params.left; bounds.width += params.left; }
    if (params.right) { bounds.width += params.right; }
